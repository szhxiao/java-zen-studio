...
SELECT * FROM t_user_basic tub WHERE loginId = 'u002' AND password ='ok';

SELECT * FROM t_user_basic tub 
	LEFT JOIN t_friend tf ON tub.id = tf.uid 
	LEFT JOIN t_user_basic tub2 ON tf.fid = tub2.id ;

SELECT fid FROM t_friend tf WHERE uid = 1;

SELECT * FROM t_topic tt WHERE author = 2;

SELECT * FROM t_user_basic tub WHERE id = 1;

SELECT fid as 'id' FROM t_friend tf WHERE uid = 1;

 

INSERT INTO `t_host_reply` (`id`, `content`, `hostReplyDate`, `author`, `reply`) 
VALUES (1, '所爱隔山海，山海皆可平','2022-03-20 18:16:40',1,3);

 SELECT * FROM t_host_reply thr WHERE reply = 3;
 
SELECT * FROM  t_reply tr WHERE topic = 8;

DELETE FROM t_reply WHERE id = ?

SELECT * FROM  t_reply tr WHERE id = 5;
