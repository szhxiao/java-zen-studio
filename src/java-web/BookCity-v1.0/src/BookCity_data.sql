DROP DATABASE IF EXISTS bookcitydb;

CREATE DATABASE bookcitydb CHAR SET utf8;
USE bookcitydb ;
CREATE TABLE `t_book` (
...
INSERT INTO `t_user`(`id`,`uname`,`password`,`email`,`role`) VALUES (1,'lina','ok','lina@sina.com.cn',0),(2,'kate','ok','hello_kate@126.com',1),(3,'鸠摩智','ok','jiujiu@126.com',0),(4,'宝2021','ok','bao2021@sohu.com.cn',0),(5,'宝2022','123','bao2022@sohu.com.cn',0);
...
SELECT * FROM `t_user` tu WHERE `uname` = 'lina' AND `password` ='ok';

SELECT * FROM `t_book tb` WHERE `bookStatus` = 0;

SELECT * FROM `t_order` to2 WHERE `orderUser` = 1;

SELECT SUM(ts.buyCount) AS totalBookCount, ts.orderBean FROM 
(
SELECT tor.id, toi.buyCount, toi.orderBean FROM t_order tor INNER JOIN t_order_item toi 
ON tor.id = toi.orderBean WHERE tor.orderUser =1 
) ts WHERE ts.orderBean=1 GROUP BY ts.orderBean ;


INSERT INTO `t_user`(`id`,`uname`,`password`,`email`,`role`) VALUES (1,'lina','ok','lina@sina.com.cn',0);
