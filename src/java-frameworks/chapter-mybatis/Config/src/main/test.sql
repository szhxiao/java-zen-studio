...
INSERT INTO `t_dept`(`pk_id`,`uk_dept_name`)
VALUES
(1,'编辑部'),
(2,'销售部'),
(3,'研发部'),
...
ALTER TABLE t_user ADD COLUMN dept_id bigint UNSIGNED;

ALTER TABLE t_user ADD CONSTRAINT fk_dept_id 
FOREIGN KEY (dept_id) REFERENCES t_dept (pk_id);
...
FROM t_user tu , t_dept td WHERE tu.dept_id = td.pk_id AND tu.pk_id = 3;
