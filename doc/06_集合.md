...
Java 集合可分为 Collection 和 Map 两个大类，Collection 接口操作单列数据，定义了存取一组对象的方法集合，其中 List 是有序、可重复的集合，Set 是无序、不可重复的集合；Map 接口操作双列数据，保存具有映射关系（key-value）的集合。
...
LinkedList 底层使用链表结构存储，对于频繁的插入删除操作效率较高。
