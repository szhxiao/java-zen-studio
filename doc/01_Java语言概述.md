...
1.  简单性
    ...
    Java 剔除了 C++中操作符操作符重载、多重继承、指针操作、虚基类等很少使用、难以理解、易混淆的特性，自动处理对象的引用和间接引用，实现垃圾自动回收，使用户不必纠结在指针、内存等复杂问题，能够集中精力做好开发。
...
1. JVM
    ...
    JVM 即 Java Virtual Machine，Java 虚拟机拥有完善的硬件架构，且屏蔽了与具体平台相关的信息，使得 Java 代码在不同平台上运行时不需要重新编译。

    JVM 主要由类装载器子系统、运行时数据区、执行引擎、本地方法接口和垃圾收集模块组成。
